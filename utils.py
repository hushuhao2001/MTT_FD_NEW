...
def epoch_with_features(mode, dataloader, net, optimizer, criterion, criterion_feature,args, aug, lbd=1,
    pooling_function=None,
    layer_idx=None,
    feat_loss=None,
    feature_strategy=None,texture=False):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    ...
    for i_batch, datum in enumerate(dataloader):
        img = datum[0].float().to(args.device)
        lab = datum[1].long().to(args.device)
        feature_syn = datum[2].float().to(args.device)
        
        # if i_batch == 0:
        # print('===================================================')
        # print('img, lab,feature_syn',img.shape, lab.shape,feature_syn.shape)
        ...
        # print('feature, feature_syn',feature.shape, feature_syn.shape)
...
#             with torch.no_grad():
#                 net.eval()
#                 if hasattr(args, 'tta') and args.tta == 'hflip':
#                     total, correct = 0, 0
#                     loss_sum = 0.0
...
#                         total += y.size(0)
#                     loss_test = loss_sum / max(total, 1)
...
def evaluate_synset(
        it_eval, net, images_train, labels_train, testloader, args, return_loss=False, texture=False
):
    net = net.to(args.device)
    images_train = images_train.to(args.device)
    labels_train = labels_train.to(args.device)
            ...
            with torch.no_grad():
                ...
                for img, target in testloader:
                    ...
                    if hasattr(args, 'tta') and args.tta == 'hflip':
                        ...
                        output_flip = net(img_flip)
                        output = (output_orig + output_flip) / 2.0  # 平均
                    else:
                        output = net(img)
                    
                    if getattr(args, "soft_label", 0.0) > 0:
                        num_classes = output.shape[1]
                        y_soft = F.one_hot(target, num_classes=num_classes).float()
                        alpha = args.soft_label
                        y_soft = y_soft * (1 - alpha) + alpha / num_classes
...
# def evaluate_synset_w_feature(it_eval, net, images_train, labels_train, feature_syn_eval,testloader, args,feature_criterion,  return_loss=False, texture=False,pooling_function=None,
#     feat_loss=None,
#     feature_strategy=None,):
#     net = net.to(args.device)
#     images_train = images_train.to(args.device)
#     labels_train = labels_train.to(args.device)
#     feature_syn_eval.to(args.device)
#     lr = float(args.lr_net)
#     Epoch = int(args.epoch_eval_train)
#     lr_schedule = [Epoch//2+1]
#     optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
...
#     dst_train = TensorDatasetFeature(images_train, labels_train,feature_syn_eval)
#     trainloader = torch.utils.data.DataLoader(dst_train, batch_size=args.batch_train, shuffle=True, num_workers=0)
...
#     for ep in tqdm.tqdm(range(Epoch+1)):
#         # loss_train, acc_train = epoch('train', trainloader, net, optimizer, criterion, args, aug=True, texture=texture)
#         loss_train, acc_train = epoch_with_features('train', trainloader, net, optimizer, criterion, feature_criterion,args, aug=True,lbd=args.lbd,
...
# def evaluate_synset_w_feature(it_eval, net, images_train, labels_train, feature_syn_eval,
#                               testloader, args, feature_criterion,
#                               return_loss=False, texture=False, pooling_function=None,
#                               feat_loss=None, feature_strategy=None):
#     net = net.to(args.device)
#     images_train = images_train.to(args.device)
#     labels_train = labels_train.to(args.device)
#     feature_syn_eval = feature_syn_eval.to(args.device)  # 修正：要接回变量
...
#             'train', trainloader, net, optimizer, criterion, feature_criterion, args,
#             aug=True, lbd=args.lbd, pooling_function=pooling_function,
#             layer_idx=args.layer_idx, feat_loss=feat_loss,
#             feature_strategy=feature_strategy, texture=texture,
#         )
#         acc_train_list.append(acc_train)
#         loss_train_list.append(loss_train)
...
def evaluate_synset_w_feature(
    ...
    images_train = images_train.to(args.device)
    labels_train = labels_train.to(args.device)
    feature_syn_eval = feature_syn_eval.to(args.device)
