...
def main(args):
    ...
    if args.batch_syn is None:
        # 🔥 智能设置batch_syn
        total_images = num_classes * args.ipc
        if total_images <= 100:
            args.batch_syn = total_images
    ...
    # 🔥 限制最大值
    if args.batch_syn > 256:
        print(f'⚠️ batch_syn={args.batch_syn} too large, setting to 256')
        args.batch_syn = 256
    ...
    print(f'Total images     : {num_classes * args.ipc}')
    print(f'Iteration        : {args.Iteration}')
    print(f'eval_it          : {args.eval_it}')
    print(f'lr_img           : {args.lr_img}')
    print(f'lr_lr            : {args.lr_lr}')
    print(f'lr_teacher       : {args.lr_teacher}')
    print(f'max_start_epoch  : {args.max_start_epoch}')
    print(f'syn_steps        : {args.syn_steps}')
    print(f'expert_epochs    : {args.expert_epochs}')
    print(f'pix_init         : {args.pix_init}')
    print(f'batch_syn        : {args.batch_syn}')
    print(f'batch_real       : {args.batch_real}')
    print(f'num_eval         : {args.num_eval}')
    print(f'epoch_eval_train : {args.epoch_eval_train}')
    print(f'zca              : {args.zca}')
    print(f'dsa              : {args.dsa}')
    print('='*80)
    
    # 🔥 参数合理性检查
    assert args.Iteration >= 5000, f"❌ Iteration too small: {args.Iteration}"
    assert args.lr_img > 0, f"❌ lr_img invalid: {args.lr_img}"
    assert args.batch_syn > 0 and args.batch_syn <= 512, f"❌ batch_syn invalid: {args.batch_syn}"
    assert args.pix_init in ['real', 'noise'], f"❌ pix_init invalid: {args.pix_init}"
    ...
    if args.load_all:
        ...
        if n == 0:
            raise AssertionError("No buffers detected at {}".format(expert_dir))
        print(f"✅ Loaded {len(buffer)} expert trajectories from {n} files")
        ...
        print("loading file {}".format(expert_files[file_idx]))
        buffer = torch.load(expert_files[file_idx])
        print(f"✅ Found {len(expert_files)} expert files, loaded {len(buffer)} experts from first file")
        if args.max_experts is not None:
            buffer = buffer[:args.max_experts]
        random.shuffle(buffer)
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it in eval_it_pool:
            for model_eval in model_eval_pool:
                ...
                print(f'model_train = {args.model}, model_eval = {model_eval}')
                print('-'*80)
                if args.dsa:
                    print('DSA augmentation strategy: \n', args.dsa_strategy)
                    print('DSA augmentation parameters: \n', args.dsa_param.__dict__)
                ...
                for it_eval in range(args.num_eval):
                    ...
                    # 修复：不打印单次评估（因为acc_train/acc_test可能是list）
                    accs_test.append(acc_test)
                    accs_train.append(acc_train)
                ...
                print(f'📈 Results: {len(accs_test)} evals, mean={acc_test_mean:.4f}%, std={acc_test_std:.4f}%')
                print(f'🏆 Best so far: {best_acc[model_eval]:.4f}%')
                print('-'*80 + '\n')
                
                # 🔥 检测训练是否异常
                if it > 500 and acc_test_mean < 15:
                    print('⚠️⚠️⚠️ WARNING: Accuracy is very low! Training may be broken!')
                    print(f'   Current acc: {acc_test_mean:.2f}%')
                    print(f'   Expected at iter {it}: ~{20 + it/1000*15:.0f}%')
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                ...
                # 🔥 修改保存路径
                save_dir = os.path.join(".", "logged_files", args.dataset, f"IPC{args.ipc}_Iter{args.Iteration}")
                ...
                if save_this_it:
                    ...
                    print(f'💾 Saved best checkpoint at iter {it}, acc={best_acc[model_eval]:.4f}%')
                ...
                if args.ipc < 50 or args.force_save:
                    ...
                    if args.dataset != "ImageNet":
                        ...
                        upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=3)
                    grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=True, scale_each=True)
                    torchvision.utils.save_image(grid, os.path.join(save_dir, f"vis_{it}.png"))
        ...
        # 🔥 检测loss异常
        if torch.isnan(grand_loss) or torch.isinf(grand_loss):
            print(f'\n❌❌❌ ERROR at iter {it}: Loss is NaN or Inf!')
            print(f'syn_lr = {syn_lr.item()}')
            print(f'param_loss = {param_loss.item()}')
            print(f'param_dist = {param_dist.item()}')
            raise RuntimeError("Training collapsed due to NaN/Inf loss")
        ...
        if it%10 == 0:
            print('%s iter = %04d, loss = %.4f, syn_lr = %.6f' % (get_time(), it, grand_loss.item(), syn_lr.item()))
            
        # 🔥 每100 iter详细报告
        if it % 100 == 0 and it > 0:
            print(f'  [Progress] {it}/{args.Iteration} ({100*it/args.Iteration:.1f}%), '
                  f'loss={grand_loss.item():.4f}')

    print('\n' + '='*80)
    print('🎉 Training completed!')
    print(f'Best Test Accuracy: {max(best_acc.values()):.4f}%')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_train', type=int, default=256, help='batch size for training networks')
    parser.add_argument('--pix_init', type=str, default='real', choices=["noise", "real"], help='initialization')
    parser.add_argument('--dsa', type=str, default='True', choices=['True', 'False'], help='DSA')
    parser.add_argument('--dsa_strategy', type=str, default='color_crop_cutout_flip_scale_rotate', help='DSA strategy')
    parser.add_argument('--data_path', type=str, default='data', help='dataset path')
    parser.add_argument('--buffer_path', type=str, default='./buffers', help='buffer path')
    parser.add_argument('--expert_epochs', type=int, default=3, help='expert epochs')
    parser.add_argument('--syn_steps', type=int, default=20, help='synthetic steps')
    parser.add_argument('--max_start_epoch', type=int, default=25, help='max start epoch')
    parser.add_argument('--zca', action='store_true', help="do ZCA whitening")
    parser.add_argument('--load_all', action='store_true', help="load all experts to RAM")
    parser.add_argument('--no_aug', type=bool, default=False, help='no augmentation')
    parser.add_argument('--texture', action='store_true', help="distill textures")
    parser.add_argument('--canvas_size', type=int, default=2, help='canvas size')
    parser.add_argument('--canvas_samples', type=int, default=1, help='canvas samples')
    parser.add_argument('--max_files', type=int, default=None, help='max expert files')
    parser.add_argument('--max_experts', type=int, default=None, help='max experts per file')
    parser.add_argument('--force_save', action='store_true', help='force save images')
